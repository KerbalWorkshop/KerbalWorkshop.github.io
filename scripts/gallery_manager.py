...
PREVIEW_DIR = BASE_DIR / "photos" / "previews" # NEW: For low-res preview images
...
PREVIEW_MAX_SIZE = (1200, 1200) # Max dimensions for the new preview images
THUMB_PREVIEW_MAX_H = 300      # Max height for the small preview in the 'Add Image' tab
ImageFile.LOAD_TRUNCATED_IMAGES = True

# Predefined metadata options for dropdowns
...
class SubjectMarkerWindow(tk.Toplevel):
    def __init__(self, parent, image_path, subjects_to_mark):
        ...
        self.subject_label = ttk.Label(self, text="", font=("Helvetica", 16, "bold"))
        ...
        ttk.Checkbutton(config_frame, text="Force Square Shape (for thumbnail area)", variable=self.square_mode_var).pack(side='left')

        actions_outer_frame = ttk.Frame(self, padding=(0, 5, 0, 0))
        ...
        self.coords_status = ttk.Label(self.coords_frame, text="◯", foreground="grey", font=("Helvetica", 14))
        self.coords_status.pack(side='left', padx=(0, 5))
        self.save_coords_button = ttk.Button(self.coords_frame, text="Save Bounding Box", command=self.save_coords, state="disabled")
        self.save_coords_button.pack(side='left')

        self.thumb_frame = ttk.Frame(button_container)
        self.thumb_frame.pack(fill='x', pady=2)
        self.thumb_status = ttk.Label(self.thumb_frame, text="◯", foreground="grey", font=("Helvetica", 14))
        self.thumb_status.pack(side='left', padx=(0, 5))
        self.save_thumb_button = ttk.Button(self.thumb_frame, text="Save Thumbnail Area", command=self.save_thumb_area, state="disabled")
        self.save_thumb_button.pack(side='left')
        
        help_text_content = ("Use this tool to define locations for subjects.\n\n"
                             "• Bounding Box: A (possibly rotated) box used to draw an ellipse on the website.\n"
                             "• Thumbnail Area: A non-rotated box that defines a crop for subject-specific galleries (like Messier).\n\n"
                             "You can save just one, or both. Click 'Next' when you are done with this subject.")
        help_label = ttk.Label(actions_outer_frame, text=help_text_content, wraplength=400, justify='left')
        help_label.grid(row=0, column=1, sticky='w', padx=20)

        nav_frame = ttk.Frame(self, padding=(0, 10))
        nav_frame.pack(fill='x', side='bottom', padx=10)
    ...
    def on_close(self):
        if messagebox.askyesno("Cancel?", "Are you sure you want to cancel marking? All markings for this session will be lost.", parent=self):
            self.results = None
            self.destroy()
...
class MultiSelectSearchableList(ttk.Frame):
    ...
    def _filter_list(self, *args):
        ...
        if query:
            filtered_items = [item for item in self.all_items if query in item['name'].lower()]
        else:
...
class DatabaseManager:
    ...
    def _run_migrations(self):
        ...
        # Check if preview_path column exists in images table
        ...
        # Check if thumbnail_box column exists and thumb_path exists in image_subjects
        cur.execute("PRAGMA table_info(image_subjects)")
        columns = [row['name'] for row in cur.fetchall()]
        if 'thumbnail_box' not in columns:
            print("Running migration: Adding 'thumbnail_box' to 'image_subjects' table...")
            self._execute("ALTER TABLE image_subjects ADD COLUMN thumbnail_box TEXT")

        if 'thumb_path' in columns:
            # This indicates an old schema. We will remove it, but more advanced
            # migration logic would be needed to preserve old thumbnail data if desired.
            # For this refactor, we are starting fresh with coordinate-based thumbs.
            print("Running migration: Removing obsolete 'thumb_path' from 'image_subjects'...")
            # Recreate table without the column
            self._execute("CREATE TABLE image_subjects_new (image_id TEXT, subject_id INTEGER, bounding_box TEXT, thumbnail_box TEXT, PRIMARY KEY(image_id, subject_id), FOREIGN KEY(image_id) REFERENCES images(id) ON DELETE CASCADE, FOREIGN KEY(subject_id) REFERENCES subjects(id) ON DELETE CASCADE)")
            self._execute("INSERT INTO image_subjects_new (image_id, subject_id, bounding_box) SELECT image_id, subject_id, bounding_box FROM image_subjects")
            self._execute("DROP TABLE image_subjects")
            self._execute("ALTER TABLE image_subjects_new RENAME TO image_subjects")

        self.conn.commit()


    def init_db_from_schema(self):
        # This function is for creating a fresh database from schema.sql
    ...
    def save_image_data(self, image_data, subject_data, gallery_ids):
        with self.conn:
            cur = self.conn.cursor()
            cur.execute("INSERT INTO images(id, label, file_path, date, integration, camera, telescope, notes, width, height, preview_path, preview_width, preview_height) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)",
                        (image_data['id'], image_data['label'], image_data['file_path'], image_data['date'], image_data['integration'],
                         ...
                         image_data['preview_path'], image_data['preview_width'], image_data['preview_height']))
    ...
    def delete_image(self, image_id):
        with self.conn:
            ...
            if main_img_row:
                if main_img_row['file_path']:
                    (BASE_DIR / main_img_row['file_path']).unlink(missing_ok=True)
                if main_img_row['preview_path']:
                    (BASE_DIR / main_img_row['preview_path']).unlink(missing_ok=True)
    ...
    def get_all_data_for_publishing(self):
        ...
        for row in subjects_cursor.fetchall():
            ...
            if image_id not in subjects: subjects[image_id] = []
        ...
        for row in galleries_cursor.fetchall():
            image_id = row['image_id']
            if image_id not in galleries: galleries[image_id] = []
        ...
        for img_id, img_data in images.items():
            result.append({
                ...
                "preview_file": img_data['preview_path'],
                "width": img_data['width'],
                "height": img_data['height'],
                "preview_width": img_data['preview_width'],
                "preview_height": img_data['preview_height'],
...
class App(tk.Tk):
    ...
    def build_tab_add(self):
        ...
        self.preview_canvas = tk.Canvas(img_frame, bg="gray20", height=THUMB_PREVIEW_MAX_H)
    ...
    # ... build_tab_browse and other UI methods ... (code for these methods will be similar to previous versions, with minor updates for new data fields if needed)
    # The methods from the previous version are mostly compatible and are included here for completeness.

    def build_tab_browse(self):
        frame = self.tab_browse
        cols = ("Label", "Date", "Integration", "Camera", "Telescope", "Galleries", "Subjects")
        self.tree = ttk.Treeview(frame, columns=cols, show="headings")
        self.tree.tag_configure('featured', background='#0058e0', foreground='white')
        for c in cols:
        ...
        self.browse_btn_container = ttk.Frame(frame); self.browse_btn_container.pack(pady=5, fill='x')
    ...
    def save_image(self):
        ...
        with Image.open(dest) as img:
            ...
            preview_dest = PREVIEW_DIR / (img_id + ".jpg")
        ...
        for subject_id, data in marker_results.items():
            ...
            thumb_bbox_str = ','.join(map(str, [c for p in data['thumb_coords'] for c in p])) if data.get('thumb_coords') else None
            subject_data[subject_id] = {'bbox': bbox_str, 'thumb_bbox': thumb_bbox_str}

        image_data = {
            'id':img_id, 'label':self.add_label.get().strip(), 'file_path':str(dest.relative_to(BASE_DIR).as_posix()), 
            ...
            'width': image_dims[0], 'height': image_dims[1],
            'preview_path': str(preview_dest.relative_to(BASE_DIR).as_posix()),
    ...
    def edit_selected(self):
        ...
        try:
            ...
            if edit_window.new_marker_data:
    ...
    def delete_selected(self):
        ...
        messagebox.showinfo("Deleted", f"Image has been deleted.")
        self.refresh_all_tabs()
